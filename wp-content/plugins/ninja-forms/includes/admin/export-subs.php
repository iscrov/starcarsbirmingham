...
function ninja_forms_export_subs_to_csv( $sub_ids = '', $return = false ){
    ...
    $par = $wpdb->get_col(
        $wpdb->prepare( "SELECT data FROM wp_ninja_forms WHERE id = '%d' ", $form_id ) // $form_id - current
    );

    $par_n = unserialize($par[0]);

    /////////////////////////////////////////////////////////

    $form_name_new = str_replace(' ', '_', mb_strtolower($par_n['form_title']));

    $filename = apply_filters( 'ninja_forms_export_subs_csv_file_name', $form_name_new.'_'.$today );
